...
def check_parameters_size(
    ...
    if not len(params['map_type_lst']) == 1 and not len(params['map_type_lst']) == len(params['map_fn_lst'] ):
        error_message("map_type")
    
    # Checks the chunk list. The list must be equal to the number of maps.
    if chunks != "auto":
        if not len(params['chunks_lst']) == 1 and not len(params['chunks_lst']) == len(params['map_fn_lst']):
    ...
    if crs is not None:
        if not len(params['crs_lst']) == 1 and not len(params['crs_lst']) == len(params['map_fn_lst']):
            error_message("crs")
        
    # Checks the no data list
    if nodata is not None:
        if not len(params['nodata_lst']) == 1 and not len(params['nodata_lst']) == len(params['map_fn_lst']):
            error_message("nodata")
        
    # Checks the var list
    if var is not None:
        if not len(params['var_lst']) == 1 and not len(params['var_lst']) == len(params['map_fn_lst']):
            error_message('var')
...
def load_floodmaps(
    ...
    **kwargs,
    ...
    # reading from the datacatalog
        ...
        if not region.empty:
            da = data_catalog.get_rasterdataset(name_catalog, variables=da_name, geom=region)
        else:
