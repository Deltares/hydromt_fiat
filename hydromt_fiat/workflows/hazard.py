...
def check_parameters(
    ...
    # Checks map path list
    if not len(params_lists["map_type_lst"]) == 1 and not len(
        params_lists["map_type_lst"]
    ) == len(map_fn_lst):
        error_message("map_type")

    # Checks the chunk list. The list must be equal to the number of maps.
    if chunks != "auto":
        if not len(params_lists["chunks_lst"]) == 1 and not len(
            params_lists["chunks_lst"]
        ) == len(map_fn_lst):
    ...
    if crs is not None:
        if not len(params_lists["crs_lst"]) == 1 and not len(
    ...
    if nodata is not None:
        if not len(params_lists["nodata_lst"]) == 1 and not len(
    ...
    # Checks the var list
    if var is not None:
        if not len(params_lists["var_lst"]) == 1 and not len(
...
def read_floodmaps(list_names, da_map_fn, idx, params_lists, params, **kwargs):
    ...
    kwargs.update(
        chunks=chunks if chunks == "auto" else params_lists["chunks_lst"][idx]
    )
...
def load_floodmaps(model, da_map_fn, name_catalog, da_name, **kwargs):
        ...
        if da_map_fn.stem == "sfincs_map":
            ...
            if not model.region.empty:
                da = model.data_catalog.get_rasterdataset(
                    da_map_fn, geom=model.region, **kwargs
                )
            else:
    ...
    # reading from the datacatalog
        ...
        if not model.region.empty:
            da = model.data_catalog.get_rasterdataset(
                name_catalog, variables=da_name, geom=model.region
            )
        else:
...
def checking_floodmaps(
    ...
    **kwargs,
    ...
    if nodata is not None:
        da_nodata = get_param(
            params_lists["nodata_lst"], map_fn_lst, "hazard", da_name, idx, "nodata"
        )
    ...
    for idx, da_map_fn in enumerate(map_fn_lst):
        ...
        check_uniqueness(
            ...
            da_name,
            {
                ...
                "chunks": "auto"
                if chunks == "auto"
                else params_lists["chunks_lst"][idx],
            },
            file_type="hazard",
            filename=da_name,
        )
